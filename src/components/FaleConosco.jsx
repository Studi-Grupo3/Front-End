...
export function FaleConosco() {
  ...
  function formatPhone(value) {
    ...
    if (len < 7) return `(${digits.substring(0, 2)}) ${digits.substring(2)}`;
    if (len <= 10)
  ...
  const validate = () => {
    ...
    if (!email.trim()) errs.email = "E-mail é obrigatório";
      ...
      if (!emailRegex.test(email.trim())) errs.email = "E-mail inválido";
    }
    if (!mensagem.trim()) errs.mensagem = "Mensagem é obrigatória";

    if (celular) {
      const digits = celular.replace(/\D/g, "");
      if (digits.length < 10) errs.celular = "Telefone incompleto";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showAlert({
        title: "Sucesso",
        text: result.message || "Enviado com sucesso!",
        icon: "success",
      });
  ...
  return (
    <div
      ...
      ref={ref}
      className={`
        min-h-[90vh] flex flex-col items-center justify-center bg-[#F8F8F8] px-4
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-md space-y-4">
        ...
        <div>
          ...
          />
          {errors.mensagem && <p className="text-red-600 text-sm mt-1">{errors.mensagem}</p>}
        </div>
        ...
        <div className="flex justify-center">
          <button
            ...
            className={`
              ...
              bg-yellow-400 text-black font-semibold py-2 rounded-md 
              border border-black cursor-pointer
              transition-all duration-300 transform
              ${
                loading
                  ? "opacity-70 cursor-not-allowed"
                  : "hover:bg-yellow-500 hover:-translate-y-1 hover:shadow-lg"
              }
            `}
          >
            {loading ? (
