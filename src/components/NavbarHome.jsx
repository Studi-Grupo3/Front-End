...
const NavbarHome = () => {
  ...
  return (
    <nav className="h-[12vh] w-full bg-[#3970B7] border-b-4 border-b-[#FECB0A] text-white px-4 lg:px-20 flex items-center justify-between text-sm relative">
      ...
      <div
        className={`absolute top-[12vh] left-0 w-full bg-[#3970B7] border-b-4 border-b-[#FECB0A] text-white flex flex-col items-center gap-6 py-6 lg:hidden transition-all duration-300 ease-in-out ${
          isOpen ? "transform translate-y-0 opacity-100" : "transform translate-y-[-100vh] opacity-0"
        }`}
      >
        <h2 className="font-semibold cursor-pointer">Sobre nós</h2>
        <h2 className="font-semibold cursor-pointer">Planos</h2>
